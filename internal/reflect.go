...
// ValueOf converts a Go value to spirit Value type. If 'v' is already a Value
// type, it is returned as is. Primitive Go values like string, rune, int, float,
// bool are converted to the right spirit Value types. Functions are converted to
...
func (fw *funcWrapper) argNames() []string {
	cleanArgName := func(t reflect.Type) string {
		return strings.Replace(t.String(), "spirit.", "", -1)
	}
...
func (fw *funcWrapper) wrapReturns(vals ...reflect.Value) (Value, error) {
	...
	wrapped := spiritValues(vals[0 : fw.lastOutIdx+1])
	if len(wrapped) == 1 {
...
func spiritValues(rvs []reflect.Value) []Value {
	var vals []Value
	for _, arg := range rvs {
